...
SearchTitle.Text = "🔍 SEARCH FEATURES"
...
FavoritesWindow = CreateWindow({Id = "Favorites", Title = "★ Favorites", Width = 340, Height = 220, Visible = false})
...
local settingsWin = CreateWindow({Id = "Settings", Title = "⚙ Settings", Width = 380, Height = 500, Visible = false})
        ...
        if input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseButton1 then
            if draggingMobile then
                ...
                if math.abs(delta.X) < 8 and math.abs(delta.Y) < 8 then
                    State.Visible = not State.Visible
                    ScreenGui.Enabled = State.Visible
                    MobileToggleBtn.Text = State.Visible and "✕" or "OT"
